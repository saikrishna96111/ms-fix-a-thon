...
class PdUtils:
    ...
    def get_inferred_fillna_columns(self, summaries):
        return [c.name for c in summaries
                if self.pd.api.types.is_numeric_dtype(c.inferred_dtype) and
                c.invalid_row_count != 0 and
                c.invalid_row_count / c.total_row_count < PdUtils.PD_INVALID_PERCENT_THRESHOLD]

    def get_inferred_drop_columns(self, summaries):
        return [c.name for c in summaries
                if (self.pd.api.types.is_numeric_dtype(c.inferred_dtype) and
                    c.invalid_row_count / c.total_row_count >= PdUtils.PD_INVALID_PERCENT_THRESHOLD) or
                (not self.pd.api.types.is_numeric_dtype(c.inferred_dtype) and not
    ...
    def get_pandas_df_summary(self, df):
        ...
        for col in df.columns:
            ...
            if (self.pd.api.types.is_numeric_dtype(inferred_dtype) or
                self.pd.api.types.is_string_dtype(inferred_dtype)) \
