...
class SubscriptionFinder(object):
    ...
    def find_through_authorization_code_flow(self, tenant, resource, authority_url):
        ...
        self.user_id = token_entry[_TOKEN_ENTRY_USER_ID]
        print("You have logged in. Now let us find all the subscriptions to which you have access...")
        if tenant is None:
            result = self._find_using_common_tenant(token_entry[_ACCESS_TOKEN], resource)
        else:
            result = self._find_using_specific_tenant(tenant, token_entry[_ACCESS_TOKEN])
    ...
    def _find_using_common_tenant(self, access_token, resource):
        ...
        for t in tenants:
            ...
            try:
                ...
                # with other tenants.
                logger.warning("Failed to authenticate to tenant '%s' due to error '%s'."
                               "Will continue to look for other tenants to find subscriptions to which you have access",
                               t.tenant_id,
                               ex)
                continue
