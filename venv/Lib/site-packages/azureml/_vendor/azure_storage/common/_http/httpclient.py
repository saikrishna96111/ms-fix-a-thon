...
class _HTTPClient(object):
    ...
    def perform_request(self, request):
        ...
        timeout = self._calculate_request_timeout(len(request.body) if request.body else 0)
    ...
    def _calculate_request_timeout(self, body_length):
        '''
        Calculating request timeout based on request body size.
        Adding additional timeout based on 5Kbyte/s transfer rate
        '''
        # In python 3.5+ timeout became tuple with request timeout and reading response timeout.
        # Request timeout also includes transfer of the request body
        if isinstance(self.timeout, tuple):
            return (self.timeout[0] + ceil(body_length / (5 * 1024)), self.timeout[1])

        return self.timeout + ceil(body_length / (5 * 1024))
